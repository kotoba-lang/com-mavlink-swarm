(ns mavlink_swarm.main-test
  "Contract + behavioral test for the mavlink_swarm-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "mavlink_swarm-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'mavlink_swarm.main-test)]
